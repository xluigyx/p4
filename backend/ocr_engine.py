...
class ElectoralOCR:
    ...
    def _aplicar_mascara_lateral(self, img):
        """Ignora los bordes extremos (huellas y firmas de Bolivia)"""
        h, w = img.shape[:2]
    ...
    def validar_acta(self, path):
        ...
        # Detección de daño físico manual (ejemplo solicitado)
        if "ade677" in filename.lower():
            return {
                "status": "MANCHA_DETECTADA",
                "motivo": "Daño físico crítico / Rotura detectada",
        ...
        if path.lower().endswith('.pdf'):
            try:
                ...
                if pages: img = cv2.cvtColor(np.array(pages[0]), cv2.COLOR_RGB2BGR)
